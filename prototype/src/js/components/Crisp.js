...
export default class Crisp extends Component {
	...
	constructor(props) {
		...
		const { customDefaultOptions } = props;

		this.state = {
			jsonError: false,
			isChanged: false,
			playerOptionsJson: JSON.stringify((customDefaultOptions == null) ? defaultOptions : customDefaultOptions, null, 4),
			playerOptions: (customDefaultOptions == null) ? defaultOptions : customDefaultOptions
		}
	...
	toggleNextVideoContent = () => {
		const options = Object.assign({}, this.state.playerOptions);
		console.log(options.playlist.length);
